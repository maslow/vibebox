...
model VibeBox {
  ...
  serverPoolId  String?        // 从哪个服务器池分配的
  apiKeyId      String?        // 分配的 API Key ID
  allocatedAt   DateTime?      // 资源分配时间
...
**状态说明**:
- **allocated**: 已从资源池分配服务器和API Key（资源预配置，立即可用）
- **running**: VibeBox正常运行，用户可连接使用
- **stopped**: 已停止，可重新启动（保留数据）
- **terminated**: 已终止，资源已释放并归还资源池
- **error**: 分配或运行出错，需要处理

**状态转换触发**:
- 支付成功: allocated → running（自动）
- 用户操作: 启动/停止/重启按钮
- 系统事件: 订阅过期、分配失败
- 自动化: 资源回收、健康检查失败

**关键优化**:
- ✅ 去掉了 provisioning 和 configuring 状态（资源池预配置）
- ✅ allocated → running 瞬间完成（<30秒）
- ✅ terminated 后资源归还池（可重复利用）

### 5.3 支付到配置流程（详细时序）

```
用户客户端          VibeBox后端         支付提供商       Happy Server      资源池
    │                   │                   │                │                │
    │ 1. 选择计划        │                   │                │                │
    ├──────────────────>│                   │                │                │
    │                   │                   │                │                │
...
- 支付确认: 实时 - 30秒
- Webhook回调: <10秒
- VibeBox配置: <30秒（资源池分配，无需安装配置）
- 客户端连接: <5秒

**关键优化**:
- ✅ 去掉了SSH连接和配置步骤（步骤15-17）
- ✅ 资源池预配置，分配即可用
- ✅ 总时间从2-5分钟缩短到<30秒
...
目的：为用户分配预配置的 VibeBox 资源

- 从服务器资源池中选择一个可用的服务器
  - 服务器已预装 Happy 守护进程
  - 服务器已预装 Claude Code
  - 服务器已预配置开发环境
  - Happy 守护进程已向 Happy Server 注册
- 从 API Key 资源池中选择一个可用的 Claude API Key
- 返回资源信息（IP、端口、root密码、API Key）
- 绑定资源到用户账户
- 更新 VibeBox 状态为 running
...
**核心理念**：
VibeBox 架构采用**资源池预配置**策略，业务层只负责**资源分配**，不涉及**资源创建**。这种分离简化了系统复杂度，加速了用户体验。
...
│  │  ✅ 已安装 Happy 守护进程                       │     │
│  │  ✅ 已安装 Claude Code                         │     │
│  │  ✅ 已配置开发环境 (git, node, python...)      │     │
│  │  ✅ 已向 Happy Server 注册                     │     │
│  │  ✅ 随时可分配给用户                            │     │
│  │                                               │     │
│  │ 状态：available | allocated | maintenance     │     │
│  └───────────────────────────────────────────────┘     │
│                                                         │
│  🔑 API Key 资源池 (API Key Pool)                       │
│  ┌───────────────────────────────────────────────┐     │
│  │ 预配置的 Claude API Keys                       │     │
│  │                                               │     │
│  │ 每个 API Key 包含：                            │     │
│  │  ✅ Claude API Key                            │     │
│  │  ✅ 每日上限配置（Claude 后台设置）            │     │
...
**VibeBox 后端（产品层）**：
- ✅ 从资源池分配资源（SELECT available resource）
- ✅ 绑定资源到用户账户
- ✅ 更新资源状态（allocated/available）
- ✅ 监控资源使用情况
- ✅ 资源回收（用户取消订阅时归还池）
...
支付成功 → 从资源池分配
    │
    ├─> 1. 查询可用服务器 (status = 'available')
    │   SELECT * FROM ServerPool WHERE status = 'available' LIMIT 1
    │
    ├─> 2. 查询可用 API Key (status = 'available')
    │   SELECT * FROM ApiKeyPool WHERE status = 'available' LIMIT 1
    │
    ├─> 3. 更新状态为已分配
    │   UPDATE status = 'allocated', userId = xxx
    │
    └─> 4. 创建 VibeBox 记录
...
订阅过期/取消 → 归还资源到池
    ...
    │
    ├─> 3. 归还到资源池
    │   UPDATE ServerPool SET status = 'available', userId = NULL
    │   UPDATE ApiKeyPool SET status = 'available', userId = NULL
    │
    └─> 4. 更新 VibeBox 状态
...
- 手动维护资源池
- 提前创建 10-20 个服务器实例
- 提前采购 10-20 个 Claude API Keys
- 手动监控库存，库存不足时手动补充

**未来（自动化）**：
- 自动监控库存水位（如：可用资源 < 5 个时告警）
- 自动扩展资源池（云服务商 API 自动创建实例）
- 自动配置新服务器（Ansible/Terraform）
- 自动健康检查和故障转移

#### 托管选项

**云服务商**:
- **中国市场**: 阿里云、腾讯云（低延迟，本地合规）
- **国际市场**: AWS EC2, DigitalOcean, Vultr, Linode

**考虑因素**:
- 成本: 按需实例 vs 预留实例
- 延迟: 用户所在地区
- 区域可用性: 多区域部署
- Happy Server 可访问性: 确保能连接到 Happy Server

#### 数据模型（可选）

如果需要在数据库中跟踪资源池状态，可以添加以下表：
...
- ✅ **简化业务逻辑**: 架构层只需分配资源，不涉及创建和配置
- ✅ **极速体验**: 用户支付后 <30秒 获得可用 Box
- ✅ **解耦关注点**: 资源创建（运维）和资源分配（业务）分离
- ✅ **前期快速验证**: 手动维护资源池，快速上线 MVP
- ✅ **后期可自动化**: 资源池管理可独立演进和自动化
...
- 2025-10-27: **重大架构调整** (v1.2) - 资源池架构
  - 确认使用预配置资源池策略（服务器池 + API Key 池）
  - 去掉 SSH 自动化配置逻辑（资源已预配置）
  - 简化状态机（去掉 provisioning/configuring 状态）
  - 缩短配置时间（2-5分钟 → <30秒）
  - 明确架构职责：业务层负责分配，运维层负责创建
  - 更新数据模型（添加资源池相关字段）
  - 完整重写 Section 9.1 资源池管理架构
