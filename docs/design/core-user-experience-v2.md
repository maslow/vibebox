...
点击"微信支付"
    ↓
弹窗内显示二维码（微信 Native 支付）
    ↓
用户用手机微信扫码
    ↓
在微信中确认支付
    ↓
Web 端通过 WebSocket 或轮询监听支付状态
    ↓
支付成功 → 弹窗消失 → 立即可用
```
...
点击"微信支付"
    ↓
调起微信 App（微信 APP 支付 / JSAPI 支付）
    ↓
在微信中确认支付
    ↓
支付完成后返回 VibeBox App（URL Scheme）
    ↓
App 主动查询订单状态
    ↓
支付成功 → 弹窗消失 → 立即可用
...
| **套餐选择** | 多套餐对比（基础版/专业版） | 单一套餐（Pro ¥299/月） |
...
**设计目的：**
在聊天详情页顶部显示用户当前订阅状态和 API 用量，便于用户随时了解剩余额度。
...
**问题：**
- API 额度用完后，用户是否可以购买叠加包（而非升级套餐）？
- 额度重置时间是按自然月（每月1号）还是按订阅周期（订阅日期）？
- 是否提供额度预警通知（如剩余 20% 时提醒）？
