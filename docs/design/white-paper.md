...
- **模块1：Box 管理**
    ...
    - **状态管理**: 用户可以在平台（App 的 Settings → Machines 或 Web）上对该 Box 执行 `启动`、`停止`、`重启` 操作。
...
- **模块2：API 用量管理**
    ...
    - **无忧体验**: MVP 阶段，平台不显示 API 用量进度、数字或百分比，让用户觉得"无忧，用不完"。用户在 Chats Tab 和 Settings 可以看到"✓ 无忧 AI 编码"的标识。
...
- **模块3：Vibe Coding 客户端体验**
    ...
    - **传统访问支持 (Fallback)**: 平台在 Settings → Machines 区块，清晰展示服务器的连接信息（如 `IP地址`, `SSH端口 (22)`, `Web端口 (80/443)`, 以及 `Root密码`或`SSH密钥`），允许用户通过传统 SSH 方式连接服务器。
- **模块4：用户账户与订阅管理**
